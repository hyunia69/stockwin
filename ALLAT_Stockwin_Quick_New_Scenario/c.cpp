...
#include "Scenaio.h"
#include "ADODB.h"
#include "ALLAT_WOWTV_Quick_Scenario.h"
...
extern "C" {
	...
	*/
	...
	{
		return new CALLAT_WOWTV_Quick_Scenario(); // CALLAT_WOWTV_Quick_Scenario 시나리오 엔진의 개체 생성
	}

	void DestroyEngine(IScenario* pComponent)
	{
		delete (CALLAT_WOWTV_Quick_Scenario *)pComponent; // CALLAT_WOWTV_Quick_Scenario 시나리오 엔진의 개체 해제
	}
...
int ALLAT_ExitSvc(int state)
{
	...
	CALLAT_WOWTV_Quick_Scenario *pScenario = (CALLAT_WOWTV_Quick_Scenario *)((*lpmt)->pScenario);
...
int ALLAT_getOrderInfo(/* [in] */int state)
{
	...
	case 0:
		...
		if (TTS_Play)
		{
			...
			return TTS_Play((*lpmt)->chanID, 92, "%s 고객님, %s에서 , 주문하신 %s의 결제하실 금액은 %d원 입니다. 서비스 중도해지시 해지일까지 이용요금과 해지수수료 10퍼센트와 제공받으신 사은품 정가가 함께 차감됩니다. 결제하시려면 1번을, 취소하시려면 2번을 눌러주세요.",
...
CALLAT_WOWTV_Quick_Scenario::CALLAT_WOWTV_Quick_Scenario()
{
	m_AdoDb = NULL;
	m_Myport = NULL;
...
int CALLAT_WOWTV_Quick_Scenario::ScenarioInit(LPMTP *Port, char *ArsType)
{
...
// 모든 시나리의 시작 부
//
int CALLAT_WOWTV_Quick_Scenario::jobArs(/* [in] */int state)
{
