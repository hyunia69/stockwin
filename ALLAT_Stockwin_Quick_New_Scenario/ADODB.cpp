...
BOOL ReadFileInternet(int ch, char *szBuffer, DWORD dSize, DWORD *pdwRead)
{
	...
	DWORD dReadSize = 0;
	xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet Calling", ch);
	if (szBuffer == NULL)
	{
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet > szBuffer=NULL", ch);
	...
	__try
	{
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet > InternetQueryDataAvailable ", ch);
		if (!InternetQueryDataAvailable(pScenario->m_hObject, &dReadSize, 0, 0))
		{
			*pdwRead = 0;
			return FALSE;
		}
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet > InternetQueryDataAvailable[%d] ", ch, dReadSize);
		...
		{
			*pdwRead = 0;
			xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet > InternetReadFile:FALSE[%d] ", ch, *pdwRead);

			return FALSE;
		}
		szBuffer[*pdwRead] = 0x00;
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet > m_hObject[%s,%d] ", ch, szBuffer, *pdwRead);
	}
	__except (GetExceptionCode() == EXCEPTION_BREAKPOINT ?
		EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH)
	{
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet Excepted of Memory ", ch);
		return FALSE;
	}
	if (*pdwRead>0) xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > ReadFileInternet [%s]", ch, szBuffer);
	return TRUE;
}
...
int Http_SSL_RetPageSend(void *data , char * szURL, char * szPostData)
{
	...
	*/
	...
	try
	{
		xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetOpen ", ch);
	...
	catch (CInternetException *e)
	{
		...
		xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetOpen[ Error Code : 0x%x, %s ]", ch,
	...
	if (pScenario->m_hSession)
	{
		...
		try
		{
			xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetCrackUrl ", ch);
			::InternetCrackUrl(szURL,  strlen(szURL) /*sizeof(URL_COMPONENTS)*/, NULL, &urlcomponent);
		}
		catch (CInternetException *e)
		{
			memset(szError, 0x00, sizeof(szError));
			xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetCrackUrl[ Error Code : 0x%x, %s ]", ch,
				e->m_dwError, e->GetErrorMessage(szError,1024));
			e->Delete();
			pScenario->m_RetCode = -1;
			return 0;
		...
		try
		{
			xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetConnect ", ch);
		...
		{
			xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetConnect[ Error Code : 0x%x, %s ]",ch,
				e->m_dwError, e->GetErrorMessage(szError, 1024));
			e->Delete();
			pScenario->m_RetCode = -1;
			return 0;
		...
		if (pScenario->m_hConnect)
		{
			...
			try
			{
				xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetAttemptConnect ", ch);
			...
			{
				memset(szError, 0x00, sizeof(szError));
				xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetAttemptConnect[ Error Code : 0x%x, %s ]", ch,
					e->m_dwError, e->GetErrorMessage(szError,1024));
				e->Delete();
				pScenario->m_RetCode = -1;
				return 0;
			...
			try
			{
				xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > HttpOpenRequest ", ch);
			...
			{
				memset(szError, 0x00, sizeof(szError));
				xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > HttpOpenRequest[ Error Code : 0x%x, %s ]", ch,
					e->m_dwError, e->GetErrorMessage(szError,1024));
				e->Delete();
				pScenario->m_RetCode = -1;
				return 0;
			...
			if (pScenario->m_hObject)
			{
				...
				xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetQueryOption ", ch);
				if (!InternetQueryOption(pScenario->m_hObject, INTERNET_OPTION_SECURITY_FLAGS,
					(LPVOID)&dwFlags, &dwBuffLen))
				{
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetQueryOption  오류: 0x%x", ch, ::GetLastError());
				}
				if (dwServiceType == AFX_INET_SERVICE_HTTPS)
				{
				...
				{
					//전체 지정이라 핸들을 지정하지 않음
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption 접속 대상 서버의 개수 지정 ", ch);
					if (!InternetSetOption(NULL, INTERNET_OPTION_MAX_CONNS_PER_SERVER, &dwVal, sizeof(dwVal)))
					{
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption(HTTP1.0) 접속 대상 서버의 개수 지정 오류 (0x%X,%d)", ch, ::GetLastError(), dwVal);

					}
					if (!InternetSetOption(NULL, INTERNET_OPTION_MAX_CONNS_PER_1_0_SERVER, &dwVal, sizeof(dwVal)))
					{
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption(HTTP1.1) 접속 대상 서버의 개수 지정 오류 (0x%X,%d)", ch, ::GetLastError(), dwVal);
					}
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption SECURITY_FLAGS 지정 ", ch);
					BOOL ret = InternetSetOption(pScenario->m_hObject, INTERNET_OPTION_SECURITY_FLAGS, &dwFlags, sizeof(dwFlags));
					if (ret)
					{
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption  오류: 0x%x [dwBuffLen:%d, sizeof(dwFlags):%d]", ch, ::GetLastError(), dwBuffLen, sizeof(dwFlags));
					}
				}
				catch (CInternetException *e)
				{
					memset(szError, 0x00, sizeof(szError));
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetSetOption[ Error Code : 0x%x, %s ]", ch,
						e->m_dwError, e->GetErrorMessage(szError,1024));
					e->Delete();
					pScenario->m_RetCode = -1;
					return 0;
				...
				if (strcmp(PostData, "POST") == 0)
				{
					try
					{
						xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario{POST] > Http_SSL_RetPageSend > HttpAddRequestHeaders ", ch);
					...
					{
						memset(szError, 0x00, sizeof(szError));
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario[POST] > Http_SSL_RetPageSend > HttpAddRequestHeaders[ Error Code : 0x%x, %s ]", ch,
							e->m_dwError, e->GetErrorMessage(szError,1024));
						e->Delete();
						pScenario->m_RetCode = -1;
						return 0;
					...
					try
					{
						xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario{POST] > Http_SSL_RetPageSend > HttpSendRequest ", ch);
					...
					{
						memset(szError, 0x00, sizeof(szError));
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario[POST] > Http_SSL_RetPageSend > HttpSendRequest[ Error Code : 0x%x, %s ]", ch,
							e->m_dwError, e->GetErrorMessage(szError,1024));
						e->Delete();
						pScenario->m_RetCode = -1;
						return 0;
				...
				else
				{
					try
					{
						xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario{GET} > Http_SSL_RetPageSend > HttpSendRequest ", ch);
						::HttpSendRequest(pScenario->m_hObject, NULL, 0L, NULL, 0);
					}
					catch (CInternetException *e)
					{
						memset(szError, 0x00, sizeof(szError));
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario[GET] > Http_SSL_RetPageSend > HttpSendRequest[ Error Code : 0x%x, %s ]", ch,
							e->m_dwError, e->GetErrorMessage(szError,1024));
						e->Delete();
						pScenario->m_RetCode = -1;
						return 0;
				...
				try
				{
					xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > HttpQueryInfo ", ch);
				...
				{
					memset(szError, 0x00, sizeof(szError));
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > HttpQueryInfo[ Error Code : 0x%x, %s ]", ch,
						e->m_dwError, e->GetErrorMessage(szError,1024));
					e->Delete();
					pScenario->m_RetCode = -1;
					return 0;
				...
				try
				{
					xprintf("[CH:%03d]ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > ReadFileInternet ", ch);
				...
				{
					memset(szError, 0x00, sizeof(szError));
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > InternetReadFile[ Error Code : 0x%x, %s ]", ch,
						e->m_dwError, e->GetErrorMessage(szError,1024));
					e->Delete();
					pScenario->m_RetCode = -1;
					return 0;
				...
				{
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > 큰버퍼 : aUtf8Buf.GetBuffer(%s,%d) ", ch, aString.GetBuffer(), aString.GetLength());
					try
					{
					...
					{
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > 큰버퍼 : 메모리 예외 발생");
						e->Delete();
					}
					catch (CException *e)
					{
						xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > 큰버퍼 : 예외 발생");
						e->Delete();
					}
				...
				{
					xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend > 적절 버퍼 :  aUtf8Buf.GetBuffer(%s,%d) ", ch, aString.GetBuffer(), aString.GetLength());
					try
					{
				...
				xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend :%s", ch, aString.GetBuffer());

				xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario > Http_SSL_RetPageSend END", ch);
				return 0;
			}
			else
			{
...
void CADODB::SetScenarion(CALLAT_Hangung_Quick_Scenario  *pScenario)
{
	m_pScenario = pScenario;
}

CADODB::CADODB(CALLAT_Hangung_Quick_Scenario  *pScenario)
{
    m_RS = NULL;
    m_CMD = NULL;
    m_CONN = NULL;
...
//AHN 20240630 개인정보보호
int CADODB::sp_getAllatOrderInfoByOrderNo(CString szDnis, CString szInputOrderNumber)
{
	if (ISOpen())
	{
		_ParameterPtr pParam;
		...
		xprintf("sp_getAllatOrderInfoByOrderNo szInputOrderNumber: %s", szInputOrderNumber);

		//2016.05.23
		//try 위치 및 범위 넓게 하자!!!
		try{
			cmd.CreateInstance("ADODB.Command");
			cmd->CommandText = "dbo.sp_getAllatOrderInfoByOrderNo";
			cmd->CommandType = adCmdStoredProc;

			pParam.CreateInstance("ADODB.Parameter");
			pParam->Name = L"@MX_ISSUE_NO";
			pParam->Type = adVarChar;
			pParam->Size = 50;//12자리에서 32 자리로 수정 2016.11.29 인입호가 해외의 번호의 것을 수용하기 위해 수정
			pParam->Direction = adParamInput;
			cmd->Parameters->Append(pParam);
			cmd->Parameters->Item[L"@MX_ISSUE_NO"]->Value = _variant_t(szInputOrderNumber.GetBuffer(pParam->Size));// 주문번호
...
int CADODB::RegOrderInfo(INFOPRODOCREQ infoReq, INFOPRODOCRES infoOrder)
{
	if (ISOpen())
	{
		...
		strSql.Format("insert into dbo.ALLAT_SHOP_ORDER\n"
			...
			TEXT(infoOrder.m_szshop_id)             // ADMIN_NAME
		);
...
int getOrderInfo_host(int holdm)
{
	...
	((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hThread = (HANDLE) _beginthreadex(NULL, 0, sp_getAllatOrderInfoByTel2, (LPVOID)(*lpmt), 0, &(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->threadID));
...
int setPayLog_host(int holdm)
{
	...
	((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_PayResult = 0;
	if (((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_AdoDb != NULL)
	{
		((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_DBAccess = -1;
		(*lpmt)->ppftbl[POST_NET].postcode = HI_OK;
	...
	((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hThread = (HANDLE)_beginthreadex(NULL, 0, setPayLogPorc, (LPVOID)(*lpmt), 0, &(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->threadID));
...
int upOrderPayState_host(int holdm)
{
	...
	((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hThread = (HANDLE)_beginthreadex(NULL, 0, upOrderPayStateProc, (LPVOID)(*lpmt), 0, &(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->threadID));
...
int getSMSOrderInfo_host(int holdm)
{
	...
	((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hThread = (HANDLE)_beginthreadex(NULL, 0, sp_getAllatOrderInfoBySMS2, (LPVOID)(*lpmt), 0, &(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->threadID));
