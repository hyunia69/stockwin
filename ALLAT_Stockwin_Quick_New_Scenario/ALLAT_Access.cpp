...
unsigned int __stdcall AllatArsPayCancleProcess(void *data)
{
	...
	xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario  AllatArsPayCancleProcess START ", ch);
	...
	xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario  AllatArsPayCancleProcess END ", ch);
	_endthreadex((unsigned int)pScenario->m_hPayThread);
	return 0;
...
unsigned int __stdcall AllatArsPayProcess(void *data)
{
	...
	xprintf("[CH:%03d] Allat ArsPayProcess end", ch);
	xprintf("[CH:%03d] ALLAT_Hangung_Quick_Scenario  AllatArsPayProcess END ", ch);
	_endthreadex((unsigned int)pScenario->m_hPayThread);
	return 0;
}
...
int  AllatPaymemt_host(int holdm)
{
	...
	if (((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hPayThread)
	{
		WaitForSingleObject(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hPayThread, INFINITE);
		CloseHandle(((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hPayThread);
		((CALLAT_Hangung_Quick_Scenario *)((*lpmt)->pScenario))->m_hPayThread = NULL;
	}
