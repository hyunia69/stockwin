...
#define CDAUTH_EXP  "01" //(card / expire )               입력 순서: card num  -> 유효기간 -> 할부 기간 -> 결제 연동 : 비인증
#define CDAUTH_ALL  "02" //(card / expire / ssn / passwd) 입력 순서: card num  -> 유효기간 -> 생년월일 -> 할부 기간 -> 비번 -> 결제 연동 : 구인증
#define CDAUTH_SNN  "03" //수기(card / expire / ssn)      입력 순서: card num  -> 유효기간 -> 생년월일 -> 할부 기간 -> 결제 연동 : 부분인증
#define CDAUTH_EXP_BILL "41" //빌키 이용 고객사 간편결제 _비인증  (체번 시 : card num  -> 유효기간 -> 할부 기간 -> 체번 연동) 결제 시 : 별도의  비번에 해당 되는 키 입력 하여 결제
#define CDAUTH_ALL_BILL "42" //빌키 이용 고객사 간편결제_구인증   (체번 시 : card num  -> 유효기간 -> 생년월일 -> 할부 기간 ->  비번 -> 체번 연동) 결제 시 : 별도의  비번에 해당 되는 키 입력 하여 결제
#define CDAUTH_SNN_BILL "43" //빌키 이용 고객사 간편결제_부분인증 (체번 시 : card num  -> 유효기간 -> 생년월일 -> 할부 기간 -> 체번 연동) 결제 시 : 별도의  비번에 해당 되는 키 입력 하여 결제
