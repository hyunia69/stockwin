...
//*********************************************************************************************************************************
// o KISA_SHA256_Transform()	: 512 비트 단위 블록의 메시지를 입력 받아 연쇄변수를 갱신하는 압축 함수로써
//						  4 라운드(64 단계)로 구성되며 8개의 연쇄변수(a, b, c, d, e, f, g, h)를 사용
// o 입력				: Message		- 입력 메시지의 포인터 변수
//						  ChainVar		- 연쇄변수의 포인터 변수
// o 출력				: 
//*********************************************************************************************************************************
void KISA_SHA256_Transform(ULONG_PTR Message, ULONG_PTR ChainVar)
{
	ULONG a, b, c, d, e, f, g, h, T1, X[64];
	ULONG j;

#define FF(a, b, c, d, e, f, g, h, j) {							\
	T1 = h + Sigma1(e) + Ch(e, f, g) + KISA_SHA256_K[j] + X[j];		\
	d += T1;													\
...
//*********************************************************************************************************************************
// o KISA_SHA256_Init()		: 연쇄변수와 길이변수를 초기화하는 함수
...
//*********************************************************************************************************************************
// o KISA_SHA256_Process()	: 임의의 길이를 가지는 입력 메시지를 512 비트 블록 단위로 나누어 압축함수를 호출하는 함수
// o 입력				: Info		 - SHA-256 구조체의 포인터 변수
...
void KISA_SHA256_Process( OUT KISA_SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	while (uDataLen >= KISA_SHA256_DIGEST_BLOCKLEN)
	{
		memcpy((UCHAR_PTR)Info->szBuffer, pszMessage, (SINT)KISA_SHA256_DIGEST_BLOCKLEN);
		KISA_SHA256_Transform((ULONG_PTR)Info->szBuffer, Info->uChainVar);
		pszMessage += KISA_SHA256_DIGEST_BLOCKLEN;
		uDataLen -= KISA_SHA256_DIGEST_BLOCKLEN;
	}
...
//*********************************************************************************************************************************
// o KISA_SHA256_Close()		: 메시지 덧붙이기와 길이 덧붙이기를 수행한 후 마지막 메시지 블록을 가지고 압축함수를 호출하는 함수
// o 입력				: Info	    - SHA-256 구조체의 포인터 변수
...
void KISA_SHA256_Close( OUT KISA_SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	Index = (Info->uLowLength >> 3) % KISA_SHA256_DIGEST_BLOCKLEN;
	Info->szBuffer[Index++] = 0x80;

	if (Index > KISA_SHA256_DIGEST_BLOCKLEN - 8)
	{
		memset((UCHAR_PTR)Info->szBuffer + Index, 0, (SINT)(KISA_SHA256_DIGEST_BLOCKLEN - Index));
		KISA_SHA256_Transform((ULONG_PTR)Info->szBuffer, Info->uChainVar);
		memset((UCHAR_PTR)Info->szBuffer, 0, (SINT)KISA_SHA256_DIGEST_BLOCKLEN - 8);
	}
	else
		memset((UCHAR_PTR)Info->szBuffer + Index, 0, (SINT)(KISA_SHA256_DIGEST_BLOCKLEN - Index - 8));
	...
	((ULONG_PTR)Info->szBuffer)[KISA_SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((ULONG_PTR)Info->szBuffer)[KISA_SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;

	KISA_SHA256_Transform((ULONG_PTR)Info->szBuffer, Info->uChainVar);

	for (i = 0; i < KISA_SHA256_DIGEST_VALUELEN; i += 4)
...
{
	KISA_SHA256_INFO info;

	KISA_SHA256_Init( &info );

	KISA_SHA256_Process( &info, pszMessage, uPlainTextLen );

	KISA_SHA256_Close( &info, pszDigest );
}
...
void KISA_SHA256_HexCode(char *pSource, char *pOut)
{
	for (int n = 0; n < 32; n++)
	{
		int Hex = (int)((char)(pSource[n]));


		sprintf_s(&pOut[n * 2],3, "%02x", Hex & 0x000000ff);
	}

	_strupr_s((char *)pOut, 65);
}