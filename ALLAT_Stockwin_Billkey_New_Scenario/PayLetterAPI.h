...
typedef struct _PL_BatchPaymentInfo {
    char memberId[PL_MAX_MEMBER_ID + 1];
    __int64 orderNo;
    char nickName[PL_MAX_NICK_NAME + 1];
    char itemName[PL_MAX_ITEM_NAME + 1];
    char pgCode[PL_MAX_PG_CODE + 1];
    char mallIdSimple[PL_MAX_MALL_ID + 1];
    char mallIdGeneral[PL_MAX_MALL_ID + 1];
    char categoryId_2nd[PL_MAX_CATEGORY_ID + 1];
    char packageId[PL_MAX_PACKAGE_ID + 1];
    char merchantId[PL_MAX_MERCHANT_ID + 1];
    int payAmt;
    int purchaseAmt;
    char couponUseFlag[PL_MAX_FLAG + 1];
    char couponName[PL_MAX_COUPON_NAME + 1];
    char bonusCashUseFlag[PL_MAX_FLAG + 1];
    int bonusCashUseAmt;
    char payAgreeFlag[PL_MAX_FLAG + 1];
    char purchaseLimitFlag[PL_MAX_FLAG + 1];
    int batchPayType;
    char serviceEndMonth[4];
    char serviceEndDay[4];
    int serviceDivDay;
    char serviceCheckFlag[PL_MAX_FLAG + 1];
    char checkCompleteTime[PL_MAX_DATETIME + 1];
    char resultCode[PL_MAX_RESULT_CODE + 1];
    char resultMessage[PL_MAX_RESULT_MESSAGE + 1];
    int errorCode;
} PL_BatchPaymentInfo;
