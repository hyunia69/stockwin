...
void PL_InitBatchPaymentInfo(PL_BatchPaymentInfo* pInfo)
{
    if (pInfo == NULL) return;
    memset(pInfo, 0x00, sizeof(PL_BatchPaymentInfo));
}
                           ...
                           PL_BatchPaymentInfo* outInfo)
{
    ...
    PL_InitBatchPaymentInfo(outInfo);
    ...
    if (eprintf) {
        eprintf("[PayLetterAPI] PL_GetBatchPaymentInfo: ========== 정기결제정보 조회 시작 ==========");
        eprintf("[PayLetterAPI] PL_GetBatchPaymentInfo: reqType=%d, reqTypeVal=%s, phoneNo=%s, arsType=%s",
                reqType, reqTypeVal ? reqTypeVal : "(null)", normalizedPhone, arsType ? arsType : "(null)");
    }

    PL_JsonBuildRequest(reqType, reqTypeVal, normalizedPhone, arsType, jsonBody, sizeof(jsonBody));

    if (!PL_HttpPost("/v1/payment/batch/getpayment/arsno_V2", jsonBody, response, sizeof(response), &statusCode)) {
        PL_Log("PL_GetBatchPaymentInfo: HTTP 요청 실패");
        return 0;
    }
    ...
    if (statusCode == 200) {
        ...
        if (strcmp(outInfo->resultCode, "0") == 0) {
            ...
            outInfo->batchPayType = PL_JsonGetInt(response, "batchPayType");
            PL_JsonGetString(response, "serviceEndMonth", outInfo->serviceEndMonth, sizeof(outInfo->serviceEndMonth));
            PL_JsonGetString(response, "serviceEndDay", outInfo->serviceEndDay, sizeof(outInfo->serviceEndDay));
            outInfo->serviceDivDay = PL_JsonGetInt(response, "serviceDivDay");

            PL_JsonGetString(response, "serviceCheckFlag", outInfo->serviceCheckFlag, sizeof(outInfo->serviceCheckFlag));
            PL_JsonGetString(response, "checkCompleteTime", outInfo->checkCompleteTime, sizeof(outInfo->checkCompleteTime));

            PL_Log("PL_GetBatchPaymentInfo: ========== 정기결제정보 조회 성공 ==========");
            PL_Log("PL_GetBatchPaymentInfo: memberId=%s, orderNo=%lld", outInfo->memberId, outInfo->orderNo);
            PL_Log("PL_GetBatchPaymentInfo: batchPayType=%d, serviceDivDay=%d", outInfo->batchPayType, outInfo->serviceDivDay);
            PL_Log("PL_GetBatchPaymentInfo: payAmt=%d, purchaseAmt=%d", outInfo->payAmt, outInfo->purchaseAmt);
            ...
            sprintf_s(outInfo->resultMessage, sizeof(outInfo->resultMessage),
                      "API 응답 resultCode: %s", outInfo->resultCode);
            PL_SetLastError(outInfo->resultMessage);
            PL_Log("PL_GetBatchPaymentInfo: 조회 실패 (resultCode=%s)", outInfo->resultCode);
            return 0;
        }
    }
    else {
        ...
        PL_SetLastError(outInfo->resultMessage);
        PL_Log("PL_GetBatchPaymentInfo: HTTP 오류 (statusCode=%d, errorCode=%d)", statusCode, outInfo->errorCode);
