...
## ⚠️ 필수 규칙 (CRITICAL RULES) ⚠️

**이 규칙들은 반드시 준수해야 합니다. 예외 없음.**

### 1. 파일 인코딩: UTF-8 with BOM 필수

```
모든 소스 코드 및 설정 파일 (.cpp, .h, .c, .ini 등)은 반드시 UTF-8 with BOM 인코딩이어야 합니다.
```
...
```
코드 변경 후 자동으로 git commit을 절대 하지 마세요.
```

- 사용자가 명시적으로 "커밋해줘", "commit", "커밋" 등을 말할 때만 커밋
- 작업 계획(Plan)에 커밋 지시를 포함하지 마세요
- 작업 완료 후 "커밋할까요?" 같은 질문도 하지 마세요
- 커밋 여부는 항상 사용자가 결정합니다
